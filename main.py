...
pygame.display.set_caption("Cowabungaa !")
...
font_name = pygame.font.match_font('Chaucer')
...
def show_go_screen():
    ...
    draw_text(screen, "Cowabungaa !", 65 , WIDTH / 2,HEIGHT / 4)
    draw_text(screen, "Do some shit!", 22 , WIDTH / 2,HEIGHT / 2)
    draw_text(screen, "press any key to start", 17 , WIDTH / 2,HEIGHT * 3 / 4)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(FPS)
...
#adding graphics
background = pygame.image.load(path.join(img_dir,"2539334976.jpg")).convert()
background_rect = background.get_rect()
player_img = pygame.image.load(path.join(img_dir,"2539334976.png")).convert()

player_mini_img = pygame.transform.scale(player_img, (25,30))
player_mini_img.set_colorkey(black)
bull_img = pygame.image.load(path.join(img_dir, "images.png")).convert()
...
for i in range(9):
    ...
    explosion_anime['lg'].append(img_lg)
    img_sm = pygame.transform.scale(img,(32,32))
    explosion_anime['sm'].append(img_sm)
...
#loading sounds
expl_sound = pygame.mixer.Sound(path.join(snd_dir,'rumble1.ogg'))
shoot_sound = pygame.mixer.Sound(path.join(snd_dir,'sfx_laser1.ogg'))
pygame.mixer.music.load(path.join(snd_dir,'nume_snd.ogg'))
...
while running:
    ...
    #keep it running on the right speed
    clock.tick(FPS)
    #events
    for event in pygame.event.get():
    ...
    #if the player dies  and explosion has finished playing
